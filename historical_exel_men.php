...
// ตรวจสอบว่าตัวแปร $d_m และ $d_y มีค่าหรือไม่
if (isset($d_m) && isset($d_y)) {
    // เพิ่มหัวข้อลงไปบนสุดของไฟล์ Excel
    $sheet->setCellValue('A1', $d_m['name']);
    ...
    // ถ้า $d_m หรือ $d_y ไม่มีค่า ให้เซ็ตค่าเริ่มต้นเป็นข้อความว่า "ไม่พบข้อมูล"
    $sheet->setCellValue('A1', 'ไม่พบข้อมูล');
}

// คำสั่ง SQL สำหรับดึงข้อมูลห้องพักสำหรับเพศชาย
$roomsQuery = "SELECT * FROM room WHERE gender = 1 ORDER BY floor, roomcode";
...
$fileName = 'รายงานย้อนหลังตึก10ชั้น(หอพักชาย).xlsx';
